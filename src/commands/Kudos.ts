...
export default class Kudos extends Command {
  constructor(client: BotClient) {
    const definition = new SlashCommandBuilder()
      .setName('kudos')
      .addMentionableOption((option) => option.setName('to').setDescription('The user or role to send a note to.').setRequired(true))
  ...
  public async run(interaction: CommandInteraction): Promise<void> {
    ...
    const mention = interaction.options.getMentionable('to', true);
