...
export default class {
  ...
  public initializeTokenHandling(client: BotClient): void {
    ...
    this.apiTokenRefreshJob = schedule.scheduleJob('0 * * * *', () => {
      ...
      if (!this.tokenValid()) {
        Logger.info(
          'Membership Portal API token no longer valid! Refreshing.',
          {
            eventType: 'manager',
            manager: 'portalAPI',
          },
  ...
  public async loginPortal(client: BotClient): Promise<void> {
    const portalAPIResponse = (await got
      .post(`${client.settings.portalAPI.url}/api/v2/auth/login`, {
        headers: {
  ...
  public tokenValid(): boolean {
    ...
    if (payload === null) {
      ...
      throw new Error(
        'JWT payload for portal API does not contain expiry date!',
      );
    }
